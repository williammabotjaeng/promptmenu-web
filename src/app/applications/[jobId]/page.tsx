...
const TalentApplicationCard = ({
  ...
  const profileImageUrl =
    userId && signedUrls && signedUrls[userId]
      ? signedUrls[userId]
  ...
  const getCountryNameFromCode = (code) => {
    ...
    const country = countries.find((country) => country.value === upperCode);
        ...
        <Grid container spacing={2}>
          ...
          >
              ...
              {status !== "pending" && (
                <Box
                  sx={{
                    ...
                    right: -5,
                    backgroundColor:
                      status === "hired" ? "success.main" : "error.main",
                    color: "white",
                    borderRadius: "50%",
                    width: 28,
                    height: 28,
            ...
            >
              Applied{" "}
              {moment(
                talent.application_data?.application_date || new Date()
              ).fromNow()}
            </Typography>
          </Grid>
            ...
            <Box sx={{ mb: 1.5 }}>
              <Box sx={{ display: "flex", alignItems: "center", mb: 0.5 }}>
                <Typography variant="h6" sx={{ fontWeight: 600, mr: 1 }}>
                  {typeof talent.firstname === "string" ? talent.firstname : ""}{" "}
                  {typeof talent.lastname === "string" ? talent.lastname : ""}
                </Typography>
              </Box>
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    >
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {typeof talent.user?.email === "string"
                          ? talent.user.email
                          : ""}
                      </Typography>
                    </Box>
                ...
                {talent.phone_number && (
                  <Grid item xs={12} sm={6}>
                    ...
                    >
                      ...
                      />
                      <Typography variant="body2" color="text.secondary">
                        {typeof talent.phone_number === "string"
                          ? talent.phone_number
                          : ""}
                      </Typography>
                    </Box>
                  ...
                  <Grid item xs={12} sm={6}>
                    ...
                    >
                      ...
                      <Box sx={{ display: "flex", alignItems: "center" }}>
                        {typeof talent.nationality === "string" && (
                          <ReactCountryFlag
                            countryCode={talent.nationality}
                            svg
                            style={{
                              width: "1.2em",
                              height: "1.2em",
                            ...
                            title={talent.nationality}
                          />
                        )}
                        <Typography variant="body2" color="text.secondary">
                          {typeof talent.nationality === "string"
                            ? getCountryNameFromCode(talent.nationality)
                            : ""}
                        </Typography>
                      </Box>
                    </Box>
                ...
                >
                  {typeof talent.application_data?.cover_letter === "string"
                    ? talent.application_data.cover_letter.length > 150
                      ? `${talent.application_data.cover_letter.substring(
                          0,
                          150
                        )}...`
                      : talent.application_data.cover_letter
          ...
          >
            ...
            >
              {talent.user?.email && (
                <MenuItem
                  onClick={() => {
                    ...
                    window.location.href = `mailto:${talent.user.email}`;
                  }}
                >
                  Send Email
                </MenuItem>
              ...
              {talent.portfolio_pdf && (
                <MenuItem
                  onClick={() => {
                    handleActionMenuClose(); /* Logic to download resume */
                  }}
                >
                  Download Resume
                </MenuItem>
              ...
              <MenuItem
                onClick={() => {
                  handleActionMenuClose(); /* Logic to add to favorites */
                }}
              >
                Add to Favorites
              </MenuItem>
  ...
  useEffect(() => {
    ...
    if (tabValue === 1) {
      filtered = filtered.filter(
        (app) => app.application_data?.status === "pending"
      );
    } else if (tabValue === 2) {
      filtered = filtered.filter(
        (app) => app.application_data?.status === "hired"
      );
    } else if (tabValue === 3) {
      filtered = filtered.filter(
        (app) => app.application_data?.status === "rejected"
      );
    }
                ...
                <Grid container spacing={2} sx={{ mb: 2 }}>
                  <Grid item xs={4}>
                    ...
                    >
                      <Typography
                        variant="h5"
                        sx={{ fontWeight: "bold", color: goldPrimary }}
                      >
                        {applications.length}
                      </Typography>
          ...
          <Grid item xs={12} md={8} lg={9}>
            ...
            >
              ...
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  Remember to finalize your talent selections at least 7 days
