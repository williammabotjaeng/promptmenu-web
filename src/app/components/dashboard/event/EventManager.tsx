...
const EventManager: React.FC<PostEventStepProps> = ({
  activeStep,
  setActiveStep,
}) => {
  const {
  ...
  const updateAllRolesWithPoster = (eventPosterName) => {
    ...
    // First pass: create a copy of all roles with their existing properties
    const copiedRoles = eventDetails.roles.map((role) => ({ ...role }));

    console.log("Event Poster Cookie:", cookies?.event_poster);
    // Second pass: update the eventPoster property on each role
    const updatedRoles = copiedRoles.map((role) => {
      role.eventPoster = eventPosterName;
      return role;
    });
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      await setCookie("event_poster", eventPosterName);
        ...
        >
          ...
          <Box sx={{ p: { xs: 2, md: 4 } }}>
            ...
            >
              <Typography
                variant="subtitle1"
                sx={{ fontWeight: "600", color: "#977342", mb: 1 }}
              >
                Ready for submission
              </Typography>
              <Typography variant="body2" sx={{ color: "#555" }}>
                Your event has been configured with{" "}
                {eventDetails?.roles?.length || 0} role(s) and is ready to be
                submitted. You can either submit it for immediate review or save
                it as a draft to continue editing later.
              </Typography>
            </Box>
          </Box>
        </Card>
