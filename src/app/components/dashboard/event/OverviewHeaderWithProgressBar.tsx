...
const OverviewHeaderWithProgressBar: React.FC<HeaderWithProgressBarProps> = ({ progressValue }) => {
  return (
    <Box sx={{ marginBottom: 4, mt: 4, width: { xs: '100%', md: '90%' }, position: 'relative' }}>
      <BorderLinearProgress variant="determinate" value={progressValue} sx={{ marginTop: 1, width: { xs: '85%' } }} />
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', marginTop: 1 }}>
        <Box sx={{ display: 'flex', flexDirection: { xs: 'row' }, alignItems: 'center', mb: { xs: 1, md: 0 } }}>
      ...
      <Link href="/dashboard">
        <CloseIcon sx={{ color: '#977342', position: 'absolute', top: 0, right: { xs: 0, md: -90 }, cursor: 'pointer' }} />
      </Link>
    </Box>
  );
};
